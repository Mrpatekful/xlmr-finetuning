...
from src.data import encode_example, decode_example, create_jsonl_loader
...
def main(cfg):
    ...
    dataset = create_jsonl_loader(cfg.batch_size, cfg.eval_data_path, encode_fn, pad_id)
    ...
    with torch.no_grad():
        for batch in tqdm(dataset, leave=False):
            ...
            lists = zip(to_list(pred_ids), to_list(label_ids), to_list(input_ids))
    ...
    for result in results:
        outputs.append("\n".join("{} {} {}".format(*values) for values in zip(*result)))

    with open(output_path, "w") as fh:
        fh.write("\n\n".join(outputs))

    command = "{} < {}".format(join(PROJECT_DIR, "scripts", "conlleval"), output_path)
