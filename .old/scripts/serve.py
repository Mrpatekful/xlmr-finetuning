...
from src.data import encode_example, decode_example

from src.model import create_pretrained

from src.serving.onnx_ml_pb2 import TensorProto
from src.serving.predict_pb2 import PredictResponse, PredictRequest
...
async def predict(query: Query):
    ...
    input_ids, label_ids = encode_example(
        tokens=tokens, labels=labels, xlmr=xlmr, label2id=label2id
    )
    ...
    for header in content_type_headers:
        request_headers = {"Content-Type": header, "Accept": "application/x-protobuf"}
    ...
    pred_ids = [
        (pred if label != -1 else -1) for pred, label in zip(pred_ids, label_ids)
    ]

    _, preds = decode_example(input_ids, pred_ids, xlmr=xlmr, id2label=id2label)
